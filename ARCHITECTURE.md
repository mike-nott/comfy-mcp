...
| `recipes/__init__.py` | Registry of image models keyed by the `model` parameter (`IMAGE_MODELS`). |
...
Video and audio are different media, so they get their own tools (`generate_video`, `generate_music`) with their own `model` keys. ComfyUI has no websocket save node for those, so their recipes write to the temp folder and the client fetches through `/view` and blanks the file afterwards.
